...
def main():
    ...
    try:
        ...
        # ---------------------------
        def _attach_refs_and_labels(rows: list[dict]) -> list[dict]:
            out = []
            for r in (rows or [])[:5]:
                rr = dict(r)
        ...
        dp["top_payables_60"] = _attach_refs_and_labels(payload.get("top_payables_60") or [])
        dp["top_receivables_60"] = _attach_refs_and_labels(payload.get("top_receivables_60") or [])

        # Pinch labels (biggest in/out)
        for pinch_key in ("pinch_14d", "pinch_30d"):
            p = dp.get(pinch_key) or {}
            for ev_key in ("biggest_out", "biggest_in"):
                ev = p.get(ev_key) or {}
                pid = str(ev.get("product_id") or "").strip()
                ref = ev.get("product_reference")
                if (not ref) and pid:
                    ref = (product_map.get(pid) or "").strip()
                    ev["product_reference"] = ref
