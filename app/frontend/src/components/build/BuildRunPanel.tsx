...
import { formatIsoDate, getStatusChipClass } from "../../lib/presentation";
    ...
    <section className="card">
      <div className="section-head">
        ...
        <span className={getStatusChipClass(buildStatus?.state === "ready" ? "Build Ready" : buildStatus?.state || "Unknown")}>
          {buildStatus?.state === "ready" ? "Build Ready" : buildStatus?.state || "idle"}
        </span>
      </div>
      ...
      <div className="build-status">
        ...
        <div>Snapshot date: {formatIsoDate(buildStatus?.snapshot_date)}</div>
      </div>
    </section>
  );
}
